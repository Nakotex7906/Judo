...
public class Combate {
    ...
    /**
     * Método estático para generar una lista de combates entre los judokas del torneo.
     */
    public static List<Combate> generarCombates(List<Judoka> participantes, Torneo torneo) {
        ...
        for (int i = 0; i < participantes.size(); i += 2) {
            if (i + 1 < participantes.size()) {
                ...
                Judoka azul = participantes.get(i + 1);
                combates.add(new Combate(blanco, azul, torneo));
            }
            // Si hay un judoka sin pareja, se puede manejar aquí
        }
        return combates;
    }
